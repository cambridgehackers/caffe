...
TYPED_TEST(ConvolutionLayerTest, TestSetup) {
  ...
  shared_ptr<Layer<Dtype> > layer(
      new ConnectalConvolutionLayer<Dtype>(layer_param));
  ...
  layer.reset(new ConnectalConvolutionLayer<Dtype>(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num(), 2);
...
TYPED_TEST(ConvolutionLayerTest, TestGradient) {
  ...
  ConnectalConvolutionLayer<Dtype> layer(layer_param);
